...
for it in range(N):
    ...
    if it%64==0 or it==N-1:
        ...
        if it>1:
            np.save(f'{ROOT}/grad_dump/mine_mean.npy', mean.astype(np.float32))
            np.save(f'{ROOT}/grad_dump/mine_std.npy', np.sqrt(m2/max(it,1)).astype(np.float32))
            np.save(f'{ROOT}/grad_dump/mine_max.npy', mx.astype(np.float32))
std=np.sqrt(m2/(N-1))
np.save(f'{ROOT}/grad_dump/mine_mean.npy', mean.astype(np.float32))
np.save(f'{ROOT}/grad_dump/mine_std.npy', std.astype(np.float32))
np.save(f'{ROOT}/grad_dump/mine_max.npy', mx.astype(np.float32))
