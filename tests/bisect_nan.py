...
import os as _os
CFG = _os.environ.get('BISECT', 'smlin4')
integ = mi.load_dict({
  'prb':    {'type':'prbvolpath','max_depth':64,'rr_depth':1064},
  'sm1':    {'type':'prbvolpath_sm','max_depth':64,'rr_depth':1064,'probes_per_segment':1},
  'smlin4': {'type':'prbvolpath_sm','max_depth':64,'rr_depth':1064,'probes_per_segment':4,'linear_cost':True},
}[CFG])
...
std=np.sqrt(m2/(N-1))
np.save(f'{ROOT}/grad_dump/bisect_mean.npy', mean.astype(np.float32))
np.save(f'{ROOT}/grad_dump/bisect_std.npy', std.astype(np.float32))
np.save(f'{ROOT}/grad_dump/bisect_max.npy', mx.astype(np.float32))
