...
for it in range(N_ITER):
    ...
    dr.backward(loss)
    # Paper's lr schedule: x0.5 after 1% of iters, x0.25 after 2% (tied to the
    # upsampling events in the reference; the lr decay is the stabilizer).
    # Albedo trains at 2x the sigma_t lr (reference param_lr_factors).
    f = 0.25 if it >= int(0.02 * N_ITER) else (0.5 if it >= int(0.01 * N_ITER) else 1.0)
...
with open(f'{OUT_DIR}/{METHOD}_batched_history.json', 'w') as f:
    json.dump(history, f, indent=1)
mi.Bitmap(np.array(mi.render(scene, sensor=TEST_SENSOR, integrator=integrator,
                             spp=256, seed=1234))).write(f'{OUT_DIR}/{METHOD}_batched_final_test.exr')
np.save(f'{OUT_DIR}/{METHOD}_batched_sigma_t.npy', np.array(params[K_SIG]))
