...
PRB={'type':'prbvolpath','max_depth':S.BASE.get('max_depth',8)}
for b in ('sphere','torus'):
    ...
    for nm,d in [('fused',dict(S.BASE)),('prb',PRB)]:
        ad,se,fd,fds=S.grad_param(scene,d,key,base.copy(),n=48)
        print(f'[{b:6s} {nm:5s}] ad={ad:+.4e}±{se:.1e} fd={fd:+.4e} ratio={ad/fd:.3f}',flush=True)
print('DIAG BOUNDARY2 DONE')
