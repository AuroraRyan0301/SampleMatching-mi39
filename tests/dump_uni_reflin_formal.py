...
integ=mi.load_dict({'type':'volpathfm_linear_sd',
...
for it in range(N):
    ...
    if it%32==0:
        ...
        np.save(f'{ROOT}/grad_dump/uni_reflin_formal_mean.npy',mean.astype(np.float32))
np.save(f'{ROOT}/grad_dump/uni_reflin_formal_mean.npy',mean.astype(np.float32))
np.save(f'{ROOT}/grad_dump/uni_reflin_formal_std.npy',np.sqrt(m2/(N-1)).astype(np.float32))
