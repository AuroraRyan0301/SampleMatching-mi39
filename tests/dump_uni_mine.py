...
N=512; SPP=8; CAM=1
for name,d in [('sm',{'type':'prbvolpath_sm','max_depth':64,'rr_depth':1064,'probes_per_segment':4}),
    ...
    for it in range(N):
        ...
        if it%64==0:
            ...
            np.save(f'{ROOT}/grad_dump/uni_{name}_mean.npy',mean.astype(np.float32))
    np.save(f'{ROOT}/grad_dump/uni_{name}_mean.npy',mean.astype(np.float32))
    np.save(f'{ROOT}/grad_dump/uni_{name}_std.npy',np.sqrt(m2/(N-1)).astype(np.float32))
