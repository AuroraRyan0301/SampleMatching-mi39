...
std=np.sqrt(m2/(N-1))
np.save(f'{ROOT}/grad_dump/ref_mean.npy', mean.astype(np.float32))
np.save(f'{ROOT}/grad_dump/ref_std.npy', std.astype(np.float32))
np.save(f'{ROOT}/grad_dump/ref_max.npy', mx.astype(np.float32))
